...
const Home = () => {
  ...
  const updateDay = (event: ChangeEvent<HTMLSelectElement>) => setDay(Number(event.currentTarget.value));
  const updatePart = (event: ChangeEvent<HTMLSelectElement>) => setPart(Number(event.currentTarget.value));
  const updatePuzzleInput = (event: ChangeEvent<HTMLTextAreaElement>) => setPuzzleInput(event.currentTarget.value);
  ...
  return (
    <Page>
      ...
      <Info>You can check the challenge for the selected day <a href={linkToChallenge()} rel="noreferrer" target="_blank">here</a>. Puzzle input for the selected day can be found <a href={linkToPuzzleInput()} rel="noreferrer" target="_blank">here</a>.</Info>
